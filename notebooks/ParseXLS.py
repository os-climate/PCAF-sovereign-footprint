    ...
    for eval_component in eval_components:
        ...
        if i == 1:
            if eval_component == "$sheet_name":
                ...
                eval_str = (
                    "df_collection[" + str(count) + "]" + "['" + eval_component + "']"
                )
            else:
...
def execute_replace_patterns_on_row(row, replace_patterns):
    for replace_pattern in replace_patterns:
        ...
        for index, col_val in enumerate(row):
            ...
            col_val = col_val.replace("'", "###")
            eval_txt = (
                "re.sub('" + pattern_1 + "','" + pattern_2 + "','" + col_val + "')"
            )
...
def parse_file(
    ...
    if file_type == "csv":
        ...
        if len(skip_rows) > 1:
            df_collection[0] = pd.read_csv(
                input_filename,
                quotechar='"',
                sep=seperator,
                engine="python",
                skiprows=int(skip_rows[1]),
        ...
        df_collection[0] = df_collection[0].loc[
            :, ~df_collection[0].columns.str.contains("^Unnamed")
        ]
        sheet_names = ["Sheet1"]
    else:
    ...
    for sheet in sheet_names:
        ...
        if not file_type == "csv":
            if len(usecols) > 0:
                if len(skip_rows) > 1:
                    df_collection[count] = pd.read_excel(
                        input_filename,
                        sheet_name=sheet,
                        usecols=usecols,
                ...
                if len(skip_rows) > 1:
                    ...
                    df_collection[count] = pd.read_excel(
                        input_filename, sheet_name=sheet, skipfooter=skip_footer
                    )
        ...
        if (len(select_columns) > 0) and (len(select_columns[0]) > 0):
            ...
            for col in select_columns:
                ...
                i = i + 1
            eval_str = (
                "df_collection[" + str(count) + "][[" + select_columns_quoted + "]]"
            )
        ...
        if len(pre_melt_var_name) > 0:
            df_collection[0] = pd.melt(
                df_collection[0],
                id_vars=pre_melt_id_vars,
        ...
        if len(rename_columns[0]) > 0:
            for i in rename_columns:
                ...
                df_collection[count].rename(
                    {column_name: column_name_new}, axis=1, inplace=True
                )

        print("column names after rename:" + df_collection[count].columns)

        try:
            chained_filter_condition = config[sheet_name][
    ...
    for key in df_collection.keys():
        ...
        if key == 0:
            if "country_iso_code" not in df_collection[key].columns:
                df_collection[0]["country_iso_code"] = cc.convert(
                    names=df_collection[0]["country_name"], to="ISO3"
                )
        else:
            if "country_iso_code" not in df_collection[key].columns:
                df_collection[key]["country_iso_code"] = cc.convert(
                    names=df_collection[key]["country_name"], to="ISO3"
                )
            df_collection[0] = pd.DataFrame.append(
                df_collection[0], df_collection[key], ignore_index=True
            )
    ...
    if len(var_name) > 0:
        df_collection[0] = pd.melt(
            df_collection[0], id_vars=id_vars, var_name=var_name, value_name="value"
        )
    print("after melt")
    print(df_collection[0])
    ...
    df_collection[0] = df_collection[0][
        df_collection[0]["country_iso_code"] != "not found"
    ]
...
def process(config_filename, output_filename, correct_quotation="no"):
    ...
    try:
        attribute_value_stored_in_cell = config["INITIAL"][
            "attribute_value_stored_in_cell"
        ]
    except:
    ...
    for input_filename in file_list:
        ...
        if (
            input_filename.endswith(".xls")
            or input_filename.endswith(".xlsx")
            or input_filename.endswith(".csv")
        ):
            print("2")
